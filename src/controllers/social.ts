...
export async function requestFriend(req: Request, res: Response) {
  ...
  if (_idErr !== undefined) return res.status(400).send({ message: "invalid user id" });
  ...
  if (updatedFriend === null)
    return res.status(404).send({ message: "user not found or they have already added you" });
  else if (friendErr !== undefined) return res.status(500).send({ message: friendErr.message });
  ...
  if (updatedUser === null)
    return res.status(404).send({ message: "user not found or they have already added you" });
  else if (userErr !== undefined) return res.status(500).send({ message: userErr.message });
...
export async function getRequestingFriends(req: Request, res: Response) {
  ...
  let { error: queryErr, value: friends } = await getFriendsWithSocialStatusRepo(_id, "REQUEST");
  if (queryErr !== undefined) return res.status(500).send({ message: queryErr.message });

  return res.status(200).send(friends.map((val) => ({ id: val._id, username: val.username })));
}
...
export async function acceptFriend(req: Request, res: Response) {
  ...
  let { error: userQueryErr, value: updatedUser } = await updateSocialStatusRepo(
    _id,
    _idFriend,
    "REQUEST",
    "ACCEPTED"
  );
  if (updatedUser === null) return res.status(404).send({ message: "user not found" });
  else if (userQueryErr !== undefined)
    return res.status(500).send({ message: userQueryErr.message });

  let { error: friendQueryErr, value: updatedFriend } = await updateSocialStatusRepo(
    _idFriend,
    _id,
    "PENDING",
    "ACCEPTED"
  );
  if (updatedFriend === null) return res.status(404).send({ message: "user not found" });
...
export async function rejectFriend(req: Request, res: Response) {
  ...
  let { error: userQueryErr, value: updatedUser } = await deleteSocialStatusRepo(_id, _idFriend);
  if (updatedUser === null) return res.status(404).send({ message: "user not found" });
  else if (userQueryErr !== undefined)
    return res.status(500).send({ message: userQueryErr.message });

  let { error: friendQueryErr, value: updatedFriend } = await deleteSocialStatusRepo(
    _idFriend,
    _id
  );
  if (updatedFriend === null) return res.status(404).send({ message: "user not found" });
...
export async function getAllFriends(req: Request, res: Response) {
  ...
  let { error: queryErr, value: friends } = await getFriendsWithSocialStatusRepo(_id, "ACCEPTED");
  if (queryErr !== undefined) return res.status(500).send({ message: queryErr.message });
